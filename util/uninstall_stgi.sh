...
#
stgi_files="/usr/local/lib/stg.bc /usr/local/include/stgi/stg.h /usr/local/lib/libSTGInstrumenter.so"
if [ ! -z $STGI_LIB ]; then
	stgi_files="$stgi_files $STGI_LIB/stg.bc $STGI_LIB/libSTGInstrumenter.so"
fi
if [ ! -z $STGI_INC ]; then
