...
"if_nested_if_else_test/nested_if_else.cpp" "if_test/if_else_if_else_min.cpp" "classify_test/classify.cpp"  "max_test/max.cpp" "fmod_test/fmod.c" "llvm_intrinsic_test/llvm.ceil+floor_test/llvm.ceil.floor_test.cpp" "llvm_intrinsic_test/llvm.exp_test/llvm.exp_test.cpp" "llvm_intrinsic_test/llvm.log_test/llvm.log_test.cpp"
 "llvm_intrinsic_test/llvm.pow_test/llvm.pow_test.cpp" "llvm_intrinsic_test/llvm.sin+cos+sqrt_test/llvm.sin+cos_test.cpp" "new_test/x.cpp")
...
do
  ...
  ./buildtest.sh "$STGI_HOME"/test_programs/"$filepath"

  mv stg-out-0 stg-out-"$filename"
  DIFF=$(diff -x '*.txt' -r -N stg-out-"$filename"  "$STGI_HOME"/test_programs/"$dir_name"/stg-expec)   # -x means exclude .txt file, -r check recursively , - N show if as full file if other absent
  #echo "$DIFF"
