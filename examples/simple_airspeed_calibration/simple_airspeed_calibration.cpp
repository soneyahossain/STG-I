...
std::array<float,maxcount> readAir;
...
int main()
{
    ...
    std::array<float,maxcount> valueArray = { 20.3, 4.5, 8.7, 8.0, 8.2 };
    readAir=valueArray;

    stg_begin_test();
    stg_symbolic_array(&readAir, "float", 5, "S");
    bool successfulCalibration = do_airspeed_calibration(1.0);
    stg_end_test();
    stg_record_test(successfulCalibration);

    //float valueArray1[5] = { 20.3, 4.5, 50.7, 8.0, 8.2 };
    std::array<float,maxcount> valueArray1 = { 20.3, 4.5, 50.7, 8.0, 8.2 };
    readAir = valueArray1;
    stg_begin_test();
    stg_symbolic_array(&readAir, "float", 5, "S");
    successfulCalibration = do_airspeed_calibration(1.0);
    stg_end_test();
    stg_record_test(successfulCalibration);
