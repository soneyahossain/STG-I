...
clang++ -std=c++14 -c -Wall -I /usr/src/gtest/include -emit-llvm -fno-discard-value-names "$filepath"/classify.cpp -o PUT.bc
opt -load="$STGI_LIB"/LLVMSTGInstrumenter.so -STGInstrumenter PUT.bc -o IPUT.bc

clang++ -std=c++14 -c -Wall -I /usr/src/gtest/include -emit-llvm -fno-discard-value-names "$filepath"/classify_test.cpp -o classify_test.bc
