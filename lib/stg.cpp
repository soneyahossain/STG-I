...
void stg_update_cmp(char* key, char* lhs, char* predicateName, char* rhs, char* type_)
{
    ...
    bool hasLeftSymbolicVar=false;
    bool hasRightSymbolicVar=false;
    ...
    if (itr != sym_state.end()){
        lvalue = itr->second;
        hasLeftSymbolicVar=true;
        }
    else{
        lvalue = loperand;
          hasLeftSymbolicVar=false;
        }
    ...
    if (itr != sym_state.end()){
        rvalue = itr->second;
         hasRightSymbolicVar=true;
         }
    else{
        rvalue = roperand;
        hasRightSymbolicVar=false;
        }
    ...
    if (hasLeftSymbolicVar||hasRightSymbolicVar) stg_state << "state[" << key << " --> " << sym_state[key] << "]\n";
}
...
void stg_update_load_pointer(void* val, void** addr)
{
   ...
   // std::cout << " load address:" << addr <<"\n";
    ...
    std::stringstream storeaddress;
    storeaddress << addr;
    std::string value = "v(" + storeaddress.str() + ")";
    ...
    key << val;
    sym_state[key.str()] = value;
    stg_state << "state[" << key.str() << " --> " << sym_state[key.str()] << "]\n";
